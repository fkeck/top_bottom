...
lakes_tab <-
  ...
  left_join(read_csv("../data/lakes_coring.csv", na = character()), by = c("LAC_CODE" = "Code")) %>% 
  mutate(Radionucleides = str_replace(Radionucleides, "137Cs", "$^{137}$Cs"),
         Radionucleides = str_replace(Radionucleides, "210Pb", "$^{210}$Pb")) %>% 
  select(LAC_CODE, everything()) %>% 
  arrange(ALTITUDE) %>% 
  set_names(c("Code", "Name", "Longitude", "Latitude", "Altitude", "Depth\n(max)",
              "Trophic status", "Coring\nyear", "XrF", "$^{14}$C", "Radionuclides"))
...
deseq_TB_AOV1 %>% 
  ...
  add_header_above(c("Taxonmic rank (Adl et al. 2019)" = 3, " " = 4)) %>% 
