...
t <- otu_tbl_replicates %>% 
  ...
  hclust() %>% 
  ape::as.phylo.hclust()
t2 <- phytools::splitTree(t, list(node = 192, bp = 0))
par(mfrow = c(1, 2))
...
p <- tax_rich_dat %>% 
  ...
  group_by(GRP2) %>% 
  mutate(GRP2_REL_RICHNESS_MEAN = mean(REL_RICHNESS_MEAN)) %>% 
  ggplot() +
  geom_col(aes(fct_reorder(ADL_RANK_2, REL_RICHNESS_MEAN),
               REL_RICHNESS_MEAN, fill = POSITION),
           position = "dodge2") +
  facet_wrap(~ fct_reorder(GRP2, GRP2_REL_RICHNESS_MEAN, .desc = TRUE),
