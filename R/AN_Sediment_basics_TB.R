...
sed_basics_plots$DNA <- ggplot(dat) +
  geom_boxplot(aes(as.numeric(as.factor(POSITION)), (ng_ADN_BY_g_DRY_SED/1000) , group = POSITION, fill = POSITION), outlier.size = 0.5)+
  ...
  xlab("") + ylab(expression(paste("DNA (", µg~g^-1, " dry sed.)"))) +
  theme_boxplot_sed

sed_basics_plots$DNA_corrected_Ctot <- ggplot(dat) +
  geom_boxplot(aes(as.numeric(as.factor(POSITION)), mug_ADN_BY_g_C_DRY_SED , group = POSITION, fill = POSITION), outlier.size = 0.5)+
  geom_line(aes(as.numeric(as.factor(POSITION)), mug_ADN_BY_g_C_DRY_SED, group = LAC), alpha = 0.2) +
  scale_x_continuous(breaks = c(1, 2), labels = c("Past (bottom)", "Recent (top)")) +
  xlab("") + ylab(expression(paste("DNA (", µg~g^-1, "TC dry sed.)"))) +
  theme_boxplot_sed

sed_basics_plots$DNA_corrected_Corg <- ggplot(dat) +
  geom_boxplot(aes(as.numeric(as.factor(POSITION)), mug_ADN_BY_g_Corg_DRY_SED , group = POSITION, fill = POSITION), outlier.size = 0.5)+
  geom_line(aes(as.numeric(as.factor(POSITION)), mug_ADN_BY_g_Corg_DRY_SED, group = LAC), alpha = 0.2) +
  scale_x_continuous(breaks = c(1, 2), labels = c("Past (bottom)", "Recent (top)")) +
  xlab("") + ylab(expression(paste("DNA (", µg~g^-1, "TOC dry sed.)"))) +
  theme_boxplot_sed
...
sed_basics_plots$COT <- ggplot(dat) +
  geom_boxplot(aes(as.numeric(as.factor(POSITION)), C_tot_perc_MS, group = POSITION, fill = POSITION), outlier.size = 0.5)+
...
sed_basics_plots$Corg <- ggplot(dat) +
  geom_boxplot(aes(as.numeric(as.factor(POSITION)), C_org_perc_MS, group = POSITION, fill = POSITION), outlier.size = 0.5)+
...
dat <- nmds_rar_list %>%
  ...
  left_join(lakes_cn, by = c("LAC", "POSITION")) %>% 
  filter(POSITION != "Bottom 2",
         !LAC %in% c("Bourget", "Léman", "Marion", "Mont Coua", "Vert", "Gentau")) %>% 
  semi_join(lakes_meta_nr, by = c("LAC", "POSITION")) %>% 
